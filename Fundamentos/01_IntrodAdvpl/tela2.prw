...
User Function Telaecr()
	...
	cEstilobutton += " border-color: rgb(255, 204, 0);"
	cEstilobutton += " font: bold 12px Arial;"
	cEstilobutton += " color: rgb(255, 204, 0);"
	...
	cEstilotget += " border-color: rgb(255, 204, 0);"
	cEstilotget += " color: rgb(255, 204, 0)"
	//css dialog
	cEstilofundo2 := " body;"
	...
	//css tfoldel
	cEstilotfolder := " a;"
	cEstilotfolder += " color rgb(255, 204, 0);"
	...
	oSayCep      := TSay():new(24,06,{||"CEP*"},oTFolder:aDialogs[2],,oTFont,,,,.T.,,,400,300,,,,,,)
	oSayCep      :SETCSS(cEstilofont)
	cTGetCep     := space(08)
	oTGetCEP     := tget():new( 24,65,{|u|if(PCount()>0,cTGetCep:=u,cTGetCep)},oTFolder:aDialogs[2],090,009,"@9 99999999",{||gravacep()},0,,oTFont3,.F.,,.T.,,.F.,,.F.,.F.,,.F.,.F.,,cTGetCep,,,, )
	oTGetCEP     :SETCSS(cEstilotget)

	oSaylogra    := TSay():new(24,275,{||"Logradouro*"},oTFolder:aDialogs[2],,oTFont,,,,.T.,,,400,300,,,,,,)
	oSaylogra    :SETCSS(cEstilofont)
	cTGetlogra   := space(50)
	oTGetlogra   := TGet():new( 24,330,{|u|if(PCount()>0,cTGetlogra:=u,cTGetlogra)},oTFolder:aDialogs[2],090,009,"@",,0,,oTFont3,.F.,,.T.,,.F.,,.F.,.F.,,.F.,.F.,,cTGetlogra,,,, )
	...
	oSayNumero   := TSay():new(58,275,{||"Numero*"},oTFolder:aDialogs[2],,oTFont,,,,.T.,,,400,300,,,,,,)
	oSayNumero   :SETCSS(cEstilofont)
	cTGetnumero  := space(50)
	oTGetnumro   := tget():new( 58,330,{|u|if(PCount()>0,cTGetnumero:=u,cTGetnumero)},oTFolder:aDialogs[2],090,009,"@9",,0,,oTFont3,.F.,,.T.,,.F.,,.F.,.F.,,.F.,.F.,,cTGetnumero,,,, )
	oTGetnumro   :SETCSS(cEstilotget)

	oSayBairro   := TSay():new(92,06,{||"Bairro"},oTFolder:aDialogs[2],,oTFont,,,,.T.,,,400,300,,,,,,)
	oSayBairro   :SETCSS(cEstilofont)
	cTGetBairro  := space(50)
	oTGetBairro  := tget():new( 92,65,{|u|if(PCount()>0,cTGetBairro:=u,cTGetBairro)},oTFolder:aDialogs[2],090,009,"@",,0,,oTFont3,.F.,,.T.,,.F.,,.F.,.F.,,.F.,.F.,,cTGetcomple,,,, )
	...
	oSayEstado   := TSay():new(126,06,{||"Estado"},oTFolder:aDialogs[2],,oTFont,,,,.T.,,,400,300,,,,,,)
	oSayEstado   :SETCSS(cEstilofont)
	oComboestado :=  TComboBox():Create(oTFolder:aDialogs[2],{|u|if(PCount()>0,cComboEstado:=u,cComboEstado)},126,65,aArrayEStado,090,009,,{||},,,,.T.,,,,,,,,,'cCombo')
...
static function gravacep()

	Local cBaseUrl 	:= "http://viacep.com.br/ws/"
	Local aHeadStr	   := {"Content-type: application/json"}
	Local oRest		   := Nil
	Local oObjJson	   := Nil
   local cMesg := "Cep não encontrado:" + CRLF as char


	oRest := FwRest():New(cBaseUrl)
	oRest:SetPath(+cTGetCep+"/json")
	...
	if oRest:Get(aHeadStr)
		...
		ElseIf "erro" $ cRetorno
         MsgAlert("cep", "cep não encotrado")
         MsgStop("cep", "cep")
         cMesg   += cTGetCep + CRLF
         MsgInfo("cep", "cep")
         cTGetCep       := space(08)
         cTGetLogra     := DecodeUTF8(space(50))   + CRLF
			cTGetcomple    := DecodeUTF8(space(50))   + CRLF
			cTGetBairro    := DecodeUTF8(space(50))   + CRLF
			cTGetCidade    := DecodeUTF8(space(50))   + CRLF
			cComboestado   := DecodeUTF8(space(50))   + CRLF
		Else

			cTGetLogra     := DecodeUTF8(oObjJson:logradouro) + CRLF
			cTGetcomple    := DecodeUTF8(oObjJson:complemento) + CRLF
			cTGetBairro    := DecodeUTF8(oObjJson:bairro) + CRLF
			cTGetCidade    := DecodeUTF8(oObjJson:localidade) + CRLF
			cComboestado   := DecodeUTF8(oObjJson:uf )+ CRLF
