...
User Function FS0006I()
   //Declarar variáveis locais
    ...
    Local lMarcar  	:= .F.
    Local aSeek   := {}
    Local bKeyF12	:= {||  U_FS006M(),oBrowse:SetInvert(.F.),oBrowse:Refresh(),oBrowse:GoTop(.T.) } //Programar a tecla F12
    ...
    If TRB->(!Eof())
        ...
        oBrowse:SetColumns(FS006TIT("TR_ID"	,"ID"		,03,"@!",0,010,0))
        oBrowse:SetColumns(FS006TIT("TR_NOME"	,"Nome"		,04,"@!",1,080,0))
        oBrowse:SetColumns(FS006TIT("TR_LOGIN","Login"	,05,"@!",1,040,0))
        oBrowse:SetColumns(FS006TIT("TR_CARGO","Cargo"	,06,"@!",1,050,0))
        oBrowse:SetColumns(FS006TIT("TR_DEPTO","Depto"	,07,"@!",1,100,0))
        oBrowse:SetColumns(FS006TIT("TR_EMAIL","E-mail"	,08,"",1,100,0))
        oBrowse:SetColumns(FS006TIT("TR_SUPER","Superior"	,09,"@!",1,020,0))
        oBrowse:SetColumns(FS006TIT("TR_POS"	,"RECNO"	,11,"@E9999999",2,20,0))
        
        //Adiciona botoes na janela
        oBrowse:AddButton("Enviar Mensagem"	, { || U_FS006M()},,,, .F., 2 )
        oBrowse:AddButton("Detalhes"		, { || MsgRun('Coletando dados de usuário(s)','Relatório',{|| U_RCFG0005() }) },,,, .F., 2 )
        oBrowse:AddButton("Legenda"			, { || FS006LEG()},,,, .F., 2 )
        
        //Indica o Code-Block executado no clique do header da coluna de marca/desmarca
        oBrowse:bAllMark := { || FS6Invert(oBrowse:Mark(),lMarcar := !lMarcar ), oBrowse:Refresh(.T.)  }
...
Static Function FS6Invert(cMarca,lMarcar)
    Local cAliasSD1 := 'TRB'
...
Static Function MenuDef()
    ...
    ADD OPTION aRot TITLE "Enviar Mensagem" ACTION "U_FS006M()"  OPERATION 6 ACCESS 0
...
Static Function FS006TIT(cCampo,cTitulo,nArrData,cPicture,nAlign,nSize,nDecimal)
    Local aColumn
    Local bData 	:= {||}
    Default nAlign 	:= 1
    Default nSize 	:= 20
